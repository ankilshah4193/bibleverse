...
const CONFIG_PATH = path.join(os.homedir(), '.agape.json');
...
// Shared by the onboarding wizard and `agape --key` (invoked with no value).
...
const promptForGroqKey = async () => {
  ...
  if (!cleanKey.startsWith('gsk_')) {
    ...
    console.log(chalk.white('Then run: ') + chalk.cyan('agape --key "your_new_key_here"') + '\n');
    return null;
  }
  return cleanKey;
};
...
const reportError = (error, { verbose = false } = {}) => {
  ...
  switch (category) {
    case 'auth':
      ...
      console.log(chalk.white('  2. Save it: ') + chalk.cyan('agape --key "gsk_..."'));
      console.log(chalk.white('     ') + chalk.dim('…or just run ') + chalk.cyan('agape --key') + chalk.dim(' and we will walk you through it.\n'));
      break;
    ...
    case 'network':
      ...
      console.log(chalk.dim('\nTip: agape also works offline — you will get the verse, just without the AI commentary.\n'));
      break;
    ...
    case 'db':
      ...
      console.log(chalk.cyan('  npm uninstall -g agape'));
      console.log(chalk.cyan('  npm install -g agape\n'));
      console.log(chalk.dim('If the problem continues, please open an issue: ') + chalk.blue.underline('https://github.com/ankilshah4193/agape/issues') + '\n');
      break;

    case 'model':
      console.log(chalk.red('\n❌ The local AI model failed to load.'));
      console.log(chalk.white('On first run, agape downloads a tiny (~25 MB) embedding model to your cache.\n'));
      ...
      console.log(chalk.white('  • Or use: ') + chalk.cyan('agape random') + chalk.dim(' — it skips the model entirely.\n'));
      break;
    ...
    default:
      ...
      console.log(chalk.dim('  ') + chalk.blue.underline('https://github.com/ankilshah4193/agape/issues') + '\n');
      break;
  }
...
//   3. User ran `agape random` and we skipped the Groq call for brevity (future use).
//
...
const renderVerseOnly = (matchedVerse, prefLang, reason = 'offline') => {
  ...
  const explanations = {
    offline: 'agape is running in Offline Mode. AI mentor commentary is disabled, but here is your guiding verse:',
  ...
  console.log(gold(`Wisdom from Book ${matchedVerse.book}: Chapter ${matchedVerse.chapter_number}, Verse ${matchedVerse.verse_number}`));
  console.log(chalk.white("--------------------------------------------------"));
  console.log(subTitleColor("English Text:"));
  ...
  const hints = {
    ...
    auth: `\nFix: run ${chalk.cyan('agape --key')} to set a new free Groq API key (no credit card needed).\n`,
...
program
  .version(pkg.version)
  .description('Bridge to Wisdom: Bible Verses for 21st Century Challenges (English + Preferred Language).')
  ...
  .action(async (cmd, options) => {
    ...
    if (options.lang) {
      // Normalize to title case so "spanish" / "SPANISH" both become "Spanish"
    ...
    if (options.key !== undefined) {
      ...
      if (options.key === true || String(options.key).trim() === '') {
        ...
        if (!cleanKey.startsWith('gsk_')) {
          ...
          console.log(chalk.white('Or run ') + chalk.cyan('agape --key') + chalk.white(' with no value to open the browser for you.\n'));
          process.exitCode = 1;
          return;
        }
      }
    ...
    if (!config.GROQ_API_KEY || !config.PREFERRED_LANGUAGE) {
      ...
      if (!config.PREFERRED_LANGUAGE) {
        console.log("");
        const userLang = await ask("Step 2 of 2: What is your preferred language for the answers? (e.g., English, Spanish, French): ");
        config.PREFERRED_LANGUAGE = toTitleCase(userLang.trim()) || 'English';
      }

      fs.writeFileSync(CONFIG_PATH, JSON.stringify(config));
      console.log(chalk.green("\n✅ Setup Complete! All settings saved."));
      console.log(chalk.white("Now try: ") + chalk.cyan('agape -t "I feel overwhelmed"') + "\n");
      return;
    }
    ...
    if (!options.topic && !isRandomMode) {
      ...
      console.log();
      console.log(chalk.yellow.bold('Agape') + chalk.white.dim('  ·  Bible Verses for 21st Century Challenges'));
      console.log(rule);

      console.log(head('\nASK'));
      console.log(row('agape -t "<question>"',  'Get guidance for your situation'));
      console.log(row('agape random',           'Draw a random verse of the day'));

      console.log(head('\nSETTINGS'));
      console.log(row('agape --lang "<language>"',  'Change your output language'));
      console.log(row('agape --key "gsk_..."',  'Update your Groq API key'));
      console.log(row('agape --key',            '…or set one interactively (opens browser)'));
      console.log(row('agape --version',        'Show the installed version'));

      console.log(head('\nEXAMPLES'));
      console.log(row('agape -t "I feel burnt out"', ''));
      console.log(row('agape -t "afraid of failing"', ''));
      console.log(row('agape --lang "Spanish"',     ''));
    ...
    if (!isRandomMode && options.topic !== undefined && options.topic.trim() === '') {
      ...
      console.log(chalk.white('Try something like: ') + chalk.cyan('agape -t "I feel overwhelmed by this release"') + '\n');
      process.exitCode = 1;
      return;
    }
    ...
    try {
      ...
      const systemPrompt = `
      You are a linguistic scholar, a philosopher, and a pastor with profound knowledge of the Bible.

      User Input: ${simulatedUserInput}
      Most Relevant Verse: Book ${matchedVerse.book}, Chapter ${matchedVerse.chapter_number}, Verse ${matchedVerse.verse_number}
      English Verse: ${matchedVerse.english_text}
      ...
      **English Verse:** [Provide original English text of the Verse]
      ${empathyInstructions}
      **Connection:** [Explain the cosmic narrative from the Bible in a detailed 2-3 line paragraph]
      ...
      **Next Action:** [Provide a high-energy practical step or reflection in a detailed 2-3 line paragraph]
      ...
      // --- 8. FINAL OUTPUT ---
      console.log(gold(`Wisdom from ${matchedVerse.book}: Chapter ${matchedVerse.chapter_number}, Verse ${matchedVerse.verse_number}`));
      console.log(chalk.white("--------------------------------------------------\n"));
      ...
      // Set DEBUG=agape in the environment to also see the stack trace.
      reportError(error, { verbose: process.env.DEBUG === 'agape' });
    }
  });
